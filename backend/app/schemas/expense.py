...
class ExpenseCommonBase(BaseSchema):
    """Expense的共用欄位定義"""
...
class ExpenseBase(ExpenseCommonBase):
    """Expense的基礎欄位定義"""
...
class ExpenseCreate(ExpenseCommonBase):
    """建立Expense 時的輸入schema"""

    splits: List[ExpenseSplitCreateItem] = Field(
        ..., min_length=1, description="分攤明細"
    )


class ExpenseCreateWithPayer(ExpenseBase):
    """建立Expense 的內部schema，會由 current_user_id 補上付款人"""
...
class ExpenseResponse(ExpenseBase, TimestampSchema, IDSchema):
    ...
    # 分攤明細
    splits: List[ExpenseSplitSimple] = Field(
        default_factory=list, description="分攤明細列表"
    )
